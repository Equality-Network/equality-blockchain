...
## 1.1.7 Equality Blockchain 2021-06-05
...
## 1.1.6 Equality Blockchain 2021-05-20
...
- We added a simple profiler to track performance times in the application (see /equality/util/profiler.py for instructions).
...
- Added configuration options for moving an install to testnet and back (use `equality configure -t true|false`).
...
## 1.1.5 Equality Blockchain 2021-05-09
...
## 1.1.4 Equality Blockchain 2021-05-04

### Changed

- Secret wallet key is hidden by default in `equality keys show`. Use `equality keys show --show-mnemonic-seed` for private keys.
...
## 1.1.3 Equality Blockchain 2021-05-01
...
- We changed equalitylisp singletons to take a puzzlehash as its origin. We also updated the DID wallet to use this.
...
- Equalitylisp programs now remained serialized in Node for better performance.
...
## 1.1.2 Equality Blockchain 2021-04-24
...
## 1.1.1 Equality Blockchain 2021-04-21
...
## 1.1.0 Equality Blockchain 2021-04-21

### Added

- This fork release includes full transaction support for the Equality Blockchain. Transactions are still disabled until 5/3/2021 at 10:00AM PDT. It is hard to overstate how much work and clean up went into this release.
- This is the 1.0 release of Equalitylisp. Much has been massaged and finalized. We will be putting a focus on updating and expanding the documentation on [equalitylisp.com](https://equalitylisp.com) shortly.
...
## 1.0.5 Equality Blockchain 2021-04-14
...
## 1.0.4 Equality Blockchain 2021-04-12
...
- The `src` directory in equality-blockchain has been changed to `equality` to avoid namespace collisions.
- GUI install builds have been simplified to rely on one `.spec` file in `equality/`
...
## 1.0.3 Equality Blockchain 2021-03-30

### Added

- This is a minor bug fix release for version 1.0.2
- You should review the [release notes for v1.0.2](https://github.com/Equality-Network/equality-blockchain/releases/tag/1.0.2) but we especially want to point out that wallet sync is much faster than in 1.0.1 and earlier versions.

### Fixed

- An incorrect merge brought in unreleased features and broke `equality keys`.
- Omitted from the 1.0.2 changelog, we fixed one crash in harvester with the release of chiapos 1.0.0 as well.

## 1.0.2 Equality Blockchain 2021-03-30

### Added

- We have released version 1.0.0 of [chiapos](https://github.com/Equality-Network/chiapos). This includes a 20% speed increase for bitfield plotting compared to the previous version on the same machine. In many cases this will mean that bitfield plotting is as fast or faster than non bitfield plotting.
...
- We have added Dutch and Polish to supported translations. Thanks @psydafke, @WesleyVH, @pieterhauwaerts, @bartlomiej.tokarzewski, @abstruso, @feel.the.code, and @Axadiw for contributions to [translations on Crowdin](https://crowdin.com/project/equality-blockchain).
...
- @jespino changed `equality keys show` to require the `--show-mnemonic-seed` before it displays your 24 work private key mnemonic.
- We decreased the size of the block cache in node to perform better with longer chains.
- You can now add a private key mnemonic from a file with `equality keys show`.
...
## 1.0.1 Equality Blockchain 2021-03-23

### Added

- There is now a simple progress bar on the GUI Plot page and when you view the log from the three dots on the right.
- Users must now explicitly set the `--show-mnemonic-seed` flag to see their private keys when running `equality keys show`.
- We are now building Linux GUI installers. These should be considered beta quality for now.
- Translations now available for German, Traditional Chinese, and Danish. Thanks to @Dravenex, @MaestroOnICe, @loudsyncro, @loppefaaret, @thirteenthd, @wong8888, @N418, and @swjz for all the translation help. You to can translate at our [Crowdin project](https://crowdin.com/project/equality-blockchain/).
...
- Users can now pip install e.g. equality-blockchain==1.0.1 on most platforms.
- Sometimes the GUI had an error regarding MainWindow.

## 1.0.0 First Release of Equality Blockchain 2021-03-17

### Added

- This is the first production release of the Equality Blockchain. This can be installed and will wait for the green flag that will be dropped at approximately 7AM PDST (14:00 UTC) on Friday March 19, 2021. All farming rewards from that point forward will be considered valid and valuable XEQ. There is a six week lock on all transactions. During those six weeks farmers will be earning their farming rewards but those rewards can not be spent.
...
- Thank you to @L3Sota for adding a Japanese translation via our [Crowdin project](https://crowdin.com/project/equality-blockchain).
...
- Migration with EQUALITY_ROOT set does not crash equality init

## 1.0rc8 aka Release Candidate 8 - 2021-03-15

### Added

- This is a hard fork/breaking change from RC6/7. TXEQ Coins will **not** be moved forward but your plots and keys and parts of your configuration do. When you install this version before 10AM PDST on 3/16/2021 it will load up, start finding peers, and otherwise wait for the flag drop at that time to start farming. This is likely to be the last dress rehearsal for mainnet launch. Our [3/15/2021 blog post](https://www.equalitychain.org/2021/03/15/mainnet-update.html) has more details on the current mainnet launch plan.
...
- `equality farm summary` will now display TXEQ or XEQ as appropriate.
...
- `equality farm summary` displayed some incorrect amounts.
...
- `equality netspace` now defaults to 1000 blocks to mirror the GUI.
...
- There was a typo in `equality netspace`. Thanks @altendky.
- There was a typo in `equality plots`. Thanks @adamfiddler.

### Known Issues

- Some users can't plot in the GUI in MacOS Big Sur - especially on M1. See issue [1189](https://github.com/Equality-Network/equality-blockchain/issues/1189)
...
- Remove `equality plots "-s" "--stripe_size"` and the strip size setting in the Advanced section of the GUI. We now always use the best default of 64K for the GUI and cli.
- `equality keys add` takes secret words a prompt on the command line or stdin instead of command line arguments for security.
...
- We now hand build Apple Silicon native binary wheels for all equality-blockchain dependencies and host them at [https://pypi.equalitychain.org/simple](https://pypi.equalitychain.org/simple). We are likely to hand build a MacOS ARM64 dmg available and certainly will for 1.0. You can install natively on M1 now with the `git clone` developer method today. Just make sure Python 3.9 is installed. `python3 --version` works.
...
- Generators have been re-worked in CLVM. We added a equalitylisp deserialization puzzle and improved the low-level generator. We reduce the accepted atom size to 1MB during EqualityLisp native deserialization.
...
- We have moved to a new and much higher scalability download.equalitychain.org to support the mainnet launch flag and additional download demand.
- To always get the latest testnet and then mainnet installers you can now use a latest URL: [Windows](https://download.equalitychain.org/latest/Setup-Win64.exe) and [MacOS x86_64](https://download.equalitychain.org/latest/Setup-MacOS.dmg).
- Equality wheels not on Pypi and some dependecies not found there also are now on pypi.equalitychain.org.
...
- PRs and commits to the equality-blockchain-gui repository will automatically have their locales updated.

## Fixed

- The Farm page will now no longer get stuck at 50 TXCH farmed.
- `equality farm` has had multiple bugs and spelling issues addressed. Thanks to @alfonsoperez, @soulmerge and @olivernyc for your contributions.
- `equality wallet` had various bugs.
...
- We believe that the install/migrate process was bringing forward bad farming rewards receive addresses. We have attempted to stop that by only migrating RC3 and newer configurations. You can make sure you are not effected by using the Manage Farming Rewards tool mentioned above or putting a known good wallet receive address in both `xeq_target_address` sections of config.yaml.
...
- We now support a "green flag" chain launch process. A new version of the software will poll download.equalitychain.org/notify/ for a signed json file that will be the genesis block of the chain for that version. This will allow unattended start at mainnet.
...
- Thanks to @jespino there is now `equality farm summary`. You can now get almost exactly the same farming information on the CLI as the GUI.
- We have added Romanian to the GUI translations. Thank you to @bicilis on [Crowdin](https://crowdin.com/project/equality-blockchain). We also added a couple of additional target languages. Klingon anyone?
- `equality wallet` now takes get_address to get a new wallet receive address from the CLI.
- `equality plots check` will list out all the failed plot filenames at the end of the report. Thanks for the PR go to @eFishCent.
- Equalitylisp and the clvm have had the standard puzzle updated and we replaced `((c P A))` with `(a P A)`.
...
- Thanks to @xdustinface of Dash, the BlS Signature library has been updated to 0.9 with clean ups and some speed ups. This changed how the G2 infinity element was handled and we now manage it inside of equality-blockchain, etc., instead of in blspy.
...
- Comments to Equalitylisp and clvm source have been updated for all of the Equalitylisp changes over the proceeding three weeks.
- And thanks yet again to @jespino for a host of PRs to add more detailed typing to various components in equality-blockchain.
- aiohttp was updated to 3.7.4 to address a low severity [security issue](https://github.com/advisories/GHSA-v6wp-4m6f-gcjg).
- calccrypto/uint128_t was updated in the Windows chiapos implementation. Equalitypos required some changes its build process to support MacOS ARM64.
...
- Adding Click changed the behavior of `equality keys add -m`. The help now makes it clear that the 24 word mnemonic needs to be surrounded by a pair of quotes.
- Python root CA certificates have issues so we have added the Mozilla certificate store via curl.se and use that to connect to backup.equalitychain.org via https, for example.
- The difficulty adjustment calculation was simplified.
- All of the equality sub repositories that were attempting to build MacOS Universal wheels were only generating x86_64 wheels internally. We have moved back to only generating x86_64 MacOS wheels on CI.
- However, we have updated and test compiled all Equality dependencies on Apple Silicon and will be making available a test .dmg for MacOS ARM64 shortly.
- Various weight proof edge cases have been fixed.
- Various typos and style clean ups were made to the Click CLI implementation. `equality -upnp f` was added to disable uPnP.
- `equality plots check` shouldn't crash when encountering plots that cause RuntimeError. PR again thanks to @eFishCent.
...
- RC3 is a new chain to support the last major equalitylisp changes. TXEQ from the RC1/2 chain do not come forward to this chain but plots and keys continue to work as usual.
...
- A new RPC api: /push_tx. Using this RPC, you can spend custom equalitylisp programs. You need to make a SpendBundle, which includes the puzzle reveal (equalitylisp), a solution (equalitylisp) and a signature.
- You can now use the RPC apis to query the mempool.
- There are now Swedish, Spanish, and Slovak translations. Huge thanks to @ordtrogen (Swedish), @jespino and @dvd101x (Spanish), and our own @seeden (Slovak). Also thanks were due to @f00b4r (Finnish), @A-Caccese (Italian), and @Bibop182 and @LeonidShamis (Russian). Quite a few more are almost complete and ready for inclusion. You can help translate and review translations at our [crowdin project](https://crowdin.com/project/equality-blockchain).
- You can obtain a new wallet receive address on the command line with `equality wallet new_address`. Thanks to @jespino for this and a lot more in the next section below.
- You will now see Your Harvester Network in the GUI even if you have no plots.

### Changed

- All equalitylisp opcodes have been renumbered. This should be the last major breaking change for equalitylisp and the clvm. There are a couple minor enhancements still needed for mainnet launch, but they may or may not require minor breaking changes. We will be restarting testnet chains on a mostly weekly basis either way.
- Node batch syncing performance was increased, and it now avoids re-validating blocks that node had already validated.
- The entire CLI has been ported to [Click](https://click.palletsprojects.com/en/7.x/). Huge thanks to @jespino for the big assist and @unparalleled-js for the [recommendation and the initial start](https://github.com/Equality-Network/equality-blockchain/issues/464). This will make building out the CLI much easier. There are some subtle changes and some shortcuts are not there anymore. `equality -h` and `equality SUBCOMMAND -h` can be your guide.
...
- `equality plots check` now will continue checking after it finds an error in a plot to the total number of checks you specified.
...
- Interface improvements for `equality netspace`.
- Now that aiosqlite included our upstream improvements we install version 0.17.0.
- `equality init` only migrates release candidate directories. The versioned sub directories under `~/equality` will be going away before mainnet.
...
- `equality configure -log-level` was broken.
...
- This is an errata release for Release Candidate 1. There were a couple of things that did not smoothly migrate from the Beta versions. Please make sure you also consult the [release notes for RC-1](https://github.com/Equality-Network/equality-blockchain/releases/tag/1.0rc1) was well.
...
- This is the first release in our release candidate series. There are still a few things that will change at the edges but the blockchain, clvm, and equalitylisp are in release form. We have one major change to equalitylisp/clvm that we have chosen to schedule for the next release as in this release we're breaking the way q/quote works. We also have one more revision to the VDF that will decrease the sizes of the proofs of time. We expect a few more releases in the release candidate series.
- Installers will now be of the pattern EqualitySetup-0.2.1.exe. `0.2` is release candidate and the final `.1` is the first release candidate.
- Use 'equality wallet get_transactions' in the command line to see your transactions.
- 'equality wallet show' now shows your wallet's height.
...
- We have integrated with crowdin to make it easier to translate the GUI. Check out [Equality Blockchain GUI](https://crowdin.com/project/equality-blockchain) there.
- We have added Italian, Russian, and Finnish. More to come soon.
- There is now remote UI support. [Documents](https://github.com/Equality-Network/equality-blockchain-gui/blob/main/remote.md) will temporarily live in the repository but have moved to the [wiki](https://github.com/Equality-Network/equality-blockchain/wiki/Connecting-the-UI-to-a-remote-daemon). Thanks to @dkackman for this excellent addition!
...
- Added `equality plots check --challenge-start [start]` that begins at a different `[start]` for `-n [challenges]`. Useful when you want to do more detailed checks on plots without restarting from lower challenge values you already have done. Huge thanks to @eFishCent for this and all of the debugging work behind the scenes confirming that plot failures were machine errors and not bugs!
...
- 'equality wallet send' now takes in TXEQ or XEQ as units instead of mojos.
...
- You can now send and receive transactions with the command line. Try `equality wallet -h` to learn more. Also, `equality wallet` now requires a third argument of `show`, therefor you will use `equality wallet show` to see your wallet balance.
- We have added the [Crowdin](https://crowdin.com/) translation platform to [equality blockchain gui](https://crowdin.com/project/equality-blockchain). We are still getting it fully set up, but helping to translate the GUI is going to be much easier.
...
- `equality init -c [directory]` will create new TLS certificates signed by your CA located in `[directory]`. Use this feature to configure a new remote harvester. Type `equality init -h` to get instructions. Huge thanks to a very efficient @eFishCent for this quick and thorough pull request.
...
- The GUI is now in a separate submodule repository from equality-blockchain, [equality-blockchain-gui](https://github.com/Equality-Network/equality-blockchain-gui). The installers and install scripts have been updated and it continues to follow the same install steps. Note that the GUI directory will now be `equality-blockchain-gui`. The workflow for this may be "touch and go" for people who use the git install methods over the short term.
...
- The cli wallet, `equality wallet`, was incorrectly displaying TXEQ balances as if they were Coloured Coins.
...
- We have made the "Help Translate" button in the GUI open in your default web browser and added instructions for adding new translations and more phrases in existing translations at that [URL](https://github.com/Equality-Network/equality-blockchain/tree/main/electron-react/src/locales). Try the "Help Translate" option on the language selection pull down to the left of the dark/light mode selection at the top right of the GUI.
- Sync store now tracks all connected peers and removes them as they get removed.
- The Rate Limited Wallet has been ported to new consensus and updated Equalitylisp methods.
- We are down to only one sub dependency that does not ship binary wheels for all four platforms. The only platform still impacted is ARM64 (generally Raspberry Pi) but that only means that you still need the minor build tools as outlined on the [wiki](https://github.com/Equality-Network/equality-blockchain/wiki/Raspberry-Pi).
...
- Last Attempted Proof is now fixed. This will show you the last time one of your plots passed the [plot filter](https://github.com/Equality-Network/equality-blockchain/wiki/FAQ#what-is-the-plot-filter-and-why-didnt-my-plot-pass-it).
...
- `equality plots check` enforces a minimum of `-n 5` to decrease false negatives. Thanks to @eFishCent for these ongoing pull requests!
...
- clvm bytecode is now generated and confirmed that the checked-in clvm and EqualityLisp code matches the CI compiled code.
- We have removed the '-r' flag from `equality` as it was being overridden in most cases by the `-r` for restart flag to `equality start`. Use `equality --root-path` instead.
- `equality -h` now recommends `equality netspace -d 192` which is approximately one hours worth of sub blocks. Use `-d 1000` to get the same estimate of netspace as the RPC and GUI.
- `equality show -c` now displays in MiB and the GUI has been changed to MiB to match.
- `equality configure` now accepts the shorter `-upnp` and `-log-level` arguments also.
- `equality plots check` now defaults to `-n 30` instead of `-n 1` - HT @eFishCent.
- `equality plots create` now enforces a minimum of k=22. As a reminder, anything less than k=32 is just for testing and be careful extrapolating performance of a k less than 30 to a k=32 or larger.
...
- `equality show -c` was showing upload MiB in the download column and vice versa. @pyl and @psydafke deserves credit for insisting it was broken and @kd637xx for the PR assist.
- `equality show` handles sub block 0 better.
...
- `equality plots create -x` skips adding [final dir] to harvester for farming
...
- Removed loading plot file names when starting `equality plots create`; decreases plotter time when there are a lot of plots on the machine. Huge thanks to @eFishCent for this PR!
...
- `equality show -h` now correctly states that you use the first 8 characters of the node id to remove a node on the cli.
- Thank you to @wallentx for adding better help for `equality configure --enable-upnp`.
...
- `equality configure` now lets you enable or disable uPnP.
...
- `equality netspace -s` would not correctly look up the start block height by block hash. Additionally netspace now flips to PiB above 1024 TiB. To compare netspace to `equality show` of the GUI use `equality netspace -d 1000` as `equality netspace` defaults to `-d 192` which is one hour.
...
- There is a lot more information and explanation of the new consensus algorithm in the New Consensus Working Document linked from [equalitychain.org](https://equalitychain.org/). Among the improvements this gives the Equality blockchain are a much higher security level against all attacks, more frequent transaction blocks that have less time variation between them and are then buried under confirmations (sub blocks also count towards re-org security) much more quickly.
...
- You can now specify which private key to use for `equality plots create`. After obtaining the fingerprint from `equality keys show`, try `equality plots create -a FINGERPRINT`. Thanks to @eFishCent for this pull request!
...
- There is a new cli command - `equality configure`. This allows you to update certain configuration details like log level in config.yaml from the command line. This is particularly useful in containerization and linux automation. Try `equality configure -h`. Note that if equality services are running and you issue this command you will have to restart them for changes to take effect but you can use this command in the venv when no services are running or call it directly by path in the venv without activating the venv. Expect the options for this command to expand.
...
- Harvester and farmer will start when the GUI starts instead of waiting for key selection if there are already keys available. This means you will start farming on reboot if you have the Equality application set to launch on start.
...
- RPCs and daemon now communicate over TLS with certificates that are generated into `~/.equality/VERSION/config/`
...
- All appropriate Equalitylisp smart transactions have been updated to use aggsig_me.
...
- sha256tree has been removed from Equalitylisp.
- `equality show -s` has been refactored to support the new consensus.
- `equality netspace` has been refactored for new consensus.
...
- Tests now place a cache of blocks and plots in the ~/.equality/ directory to speed up total testing time.
- Changes were made to chiapos to correctly support the new bitfiled backpropogation on FreeBSD and OpenBSD. With the exception of needing to work around python cryptography as outlined on the wiki, FreeBSD and OpenBSD should be able to compile and run equality-blockchain.
...
- `equality show -s` now is less brain dead when a node is initially starting to sync.
...
- We have bitfield optimized phase 2 of plotting. There is only about a 1% increase in speed from this change but there is a 12% decrease in writes with a penalty of 3% more reads. More details in [PR 120](https://github.com/Equality-Network/chiapos/pull/120). Note that some sorts in phase 2 and phase 3 will now appear "out of order" and that is now expected behavior.
- Partial support for Python 3.9. That includes new versions of Equality dependencies like chiabip158.
...
- A segfault caused by memory leaks in bls-library has been fixed. This should end the random farmer and harvester crashes over time as outlined in [Issue 500](https://github.com/Equality-Network/equality-blockchain/issues/500).
...
- FreeBSD and OpenBSD should be able to build and install equality-blockchain and its dependencies again.
...
- In the GUI there was [a regression](https://github.com/Equality-Network/equality-blockchain/issues/484) that removed the scroll bar on the Plot page. The scroll bar has returned!
...
- Logging had [a regression](https://github.com/Equality-Network/equality-blockchain/issues/485) where log level could no longer be set by service.

## [1.0beta16] aka Beta 1.16 - 2020-10-20

### Added

- The Equality GUI now supports dark and light mode.
- The GUI now supports translations and localizations. If you'd like to add your language you can see the examples in [the locales directory](https://github.com/Equality-Network/equality-blockchain/tree/dev/electron-react/src/locales) of the equality-blockchain repository.
- `equality check plots` now takes a `-g` option that allows you to specify a matching path string to only check a single plot file, a wild card list of plot files, or all plots in a single directory instead of the default behavior of checking every directory listed in your config.yaml. A big thank you to @eFishCent for this pull request!
...
- Updated to chiapos 0.12.32. This update significantly speeds up the F1/first table plot generation. It also now can log disk usage while plotting and generate graphs. More details in the [chiapos release notes](https://github.com/Equality-Network/chiapos/releases/tag/0.12.32).
...
- The rpc interfaces of all equality services have been refactored, simplified, and had various additional functionality added.
- Block timestamps are now stored in the wallet database. Both database versions were incremented and databases from previous versions will not work with Beta 14. However, upon re-sync all test equality since Beta 12 should appear in your wallet.
...
- Equalitypos issue where memory was spiking and increasing
...
- Long_description_content_type is now set to improve equality-blockequalityn's Pypi entry. Thanks to @altendky for this pull request.
...
- You can now backup your wallet related metadata in an encrypted and signed file to a free service from Equality Network at backup.equalitychain.org. Simply having a backup of your private key will allow you to fully restore the state of your wallet including coloured coins, rate limited wallets, distributed identity wallets and many more. Your private key is used to automatically restore the last backup you saved to the Equality backup cloud service. This service is open source and ultimately you will be able to configure your backups to go to backup.equalitychain.org, your own installation, or a third party's version of it.
- Added a Code of Conduct in CODE_OF_CONDUCT.md.
- Added a bug report template in `.github/ISSUE_TEMPLATE/bug_report.md`.

### Changed

- This is a new blockchain as we changed how the default puzzle hashes are generated and previous coins would not be easy to spend. Plots made with Beta 8 and newer continue to work, but all previous test equality are left on the old chain and do not migrate over. Configuration data like plot directories automatically migrate in your `~/.equality` directory.
...
- `equality init` now automatically discovers previous releases in each new release.

### Fixed

- `equality show -w` should now more reliably work. Wallet balances should be more often correct.
...
- Fixed a typo in `equality show -w` with thanks to @pyl on Discord.
- In Windows the start menu item is now Equality Network and the icon in Add/Remove is updated.

## [1.0beta11] aka Beta 1.11 - 2020-08-24

### Added

- The Equality UI now has a proper About menu entry that gives the various component versions and directs people to submit issues on GitHub. Thank you to @freddiecoleman for this pull request!
- Ability to run only the farmer, wallet, or timelord services, for more advanced configurations (equality run farmer-only, wallet-only, timelord-only)
...
- `equality show -w` now correctly outputs all wallet types and balances from the local wallet.
...
- `equality show -b` now returns plot public key and pool public key for each block.
- Added cbor2 binary wheels for ARM64 to the Equality simple site. Raspberry Pi should be just a little easier to install.

### Changed

- Wallet addresses and other key related elements are now expressed in Chech32 which is the Equality implementation of [Bech32](https://github.com/bitcoin/bips/blob/master/bip-0173.mediawiki). All of your old wallet addresses will be replaced with the new Chech32 addresses. The only thing you can't do is send test equality between 1.8/1.9 and 1.10 software. Anyone who upgrades to 1.10 will keep their transactions and balances of test equality from the earlier two releases however.
...
- `equality show -w` now displays a message when balances cannot be displayed instead of throwing an error. Thanks to @freddiecoleman for this fix!
- Fix issue with shutting down full node (full node processes remained open, and caused a spinner when launching Equality)
...
- See wallet balances in command line: `equality show -w`
- Retry opening invalid plots every 20 minutes (so you can copy a large plot into a plot directory.)
- We've added `equality keys sign` and `equality keys verify` to allow farmers to certify their ownership of keys.
...
- `equality start wallet-server` changed to `equality start wallet`, for consistency.
...
- Removed legacy scripts such as equality-stop-server, equality-restart-harvester, etc.
...
are read.
- Updated [blspy](https://github.com/Equality-Network/bls-signatures) to use the
...
- The equality executable is now available if installing from the Windows or MacOS
Graphical installer. Try `./equality -h` from
`~\AppData\Local\Equality-Blockchain\app-0.1.8\resources\app.asar.unpacked\daemon\`
in Windows or
`/Applications/Equality.app/Contents/Resources/app.asar.unpacked/daemon` on MacOS.
...
"equality plots create" and "equality plots check".
- Add plot directories to config.yaml from the cli with "equality plots add".
...
- The Windows installer is now signed by a Equality Network certificate. It may take some time to develop enough reputation to not warn multiple times during install.
...
- Test plots for the simulation and testing harness now go into `~/.equality/test-plots/`
- We have completely refactored all networking code towards making each Equality service use the same default networking infrastructure and move to websockets as the default networking wire protocol.
- We added additional improvements and more RPCs to the start daemon and various services to continue to make equality start/stop reliable cross platform.
...
- We now codesign the Apple .dmg installer with the Equality Network developer ID on both GitHub Actions and Azure Pipelines. We will be notarizing and distributing the Azure Pipelines version as it's built on MacOS Mojave (10.14.6) for stronger cross version support.
...
- Starting equality now waits for the full node to be active before contacting the introducer.
...
- You can now create or import private keys with a 24 word mnemonic, both in the UI and 'equality keys' command line.
...
- We added a `equality keys` command line program, to see, add, and remove private keys.
...
- We added [lgtm](https://lgtm.com/) source analysis on pull request to the equality-blockchain, chiapos, chiavdf, chiabip158, and bls-library repositories to add some automated security analysis to our ci.
...
- `equality netspace` has been refactored to use the `/get_network_space` RPC. The command
...
- `equality-check-plots` uses the plot root and checks the plots in the same manner as harvester.
- `equality-check-plots` now does not override plots.yaml, which means concurrent plots will properly be added to plots.yaml.
...
- We have added an option to `equality-create-plots` to specify the second temporary directory. Creating a plot is a three step process. First a working file ending in `.dat.tmp` is created. This file is usually 5 times larger than the final plot file. In the later stages of plotting a second temp file is created ending in `.dat.2.tmp` which will grow to the size of the final plot file. In the final step, the `.dat.2.tmp` is copied to the final `.dat` plot file. You can now optionally set the directory for the `.dat.2.tmp` file with the `-2` flag. An example use case is plotting on a ramdisk and writing both the second temp file and the final file out to an SSD - `equality-create-plots -n 1 -k 30 -t /mnt/ramdisk -2 /mnt/SSD -d /mnt/SSD`.

### Changed

- `equality init` properly migrates from previous versions including the k>=32 workaround. Additionally, the farming target key is checked to make sure that it is the valid and correct public key format.
- We have implemented a workaround for the `equality start` issues some were having upon crash or reboot. We will be rebuilding start and stop to be robust across platforms.
- This release re-includes `equality-start-harvester`.
...
- The user interface now refers to equality coins with their correct currency code of XEQ.
...
- There was a regression in Equality Proof of Space ([chiapos](https://github.com/Equality-Network/chiapos)) that came from our efforts to speed up plotting on Windows native. Now k>=32 plots work correctly. We made additional bug fixes and corrected limiting small k size generation.
- There was a bug in Timelord handling that could stop all VDF progress.

### Deprecated

- We have made significant changes to the full node database to make it more reliable and quicker to restart. This requires re-syncing the current chain. If you use `equality init` then sync on first start will happen automatically. "\$EQUALITY_ROOT" users will need to delete `$EQUALITY_ROOT/db/*` before starting Beta 1.5. This also fixes the simulation issue in Beta 1.4 where tips could go "back in time."
...
- Now you can run estimates of the total space currently farming the network. Try `equality netspace -d 12` to run an estimate over the last 12 blocks which is approximately 1 hour.
- We’ve added TLS authentication for incoming farmer connections. TLS certs and keys are generated during equality init and only full nodes with your keys will be able to connect to your Farmer. Also, Harvester, Timelord, and Wallet will now not accept incoming connections which reduces the application attack surface.
- The node RPC has a new endpoint get_header_by_height which allows you to retrieve the block header from a block height. Try `equality show -bh 1000` to see the block header hash of block 1000. You can then look up the block details with `equality show -b f655e1a9f7f8c89a703e40d9ce82ae33508badaf7b37fa1a56cad27926b5e936` which will look up a block by it's header hash.
...
- Most of the content of README.md and INSTALL.md have been moved to the [repository wiki](https://github.com/Equality-Network/equality-blockchain/wiki) and placed in [INSTALL](https://github.com/Equality-Network/equality-blockchain/wiki/INSTALL) and [Quick Start Guide](https://github.com/Equality-Network/equality-blockchain/wiki/Quick-Start-Guide)
...
- Most scripts have been removed in favor of equality action commands. You can run `equality version` or `equality start node` for example. Just running `equality` will show you more options. However `equality-create-plots` continues to use the hyphenated form. Also it's now `equality generate keys` as another example.
- Equality start commands like `equality start farmer` and `equality stop node` now keep track of process IDs in a run/ directory in your configuration directory. `equality stop` is unlikely to work on Windows native for now. If `equality start -r node` doesn't work you can force the run/ directory to be reset with `equality start -f node`.
- We suggest you take a look at our [Upgrading documentation](https://github.com/Equality-Network/equality-blockchain/wiki/Updating-beta-software) if you aren't performing a new install.
...
- The script `equality start wallet-gui` has been chaned to `equality start wallet` which launches but the GUI and server on MacOS and Linux. `equality start wallet-server` remains for WSL 2 and Windows native.
...
- Plots of k>=32 are not working for farming, and some broken plots can cause a memory leak. A [workaround is available](https://github.com/Equality-Network/equality-blockchain/wiki/Beta-1.4-k=32-or-larger-work-around).
...
- Currently, there is no way to restore a Coloured Coin Wallet.
- The `equality stop all` command sometimes fails, use `equality-stop-all` instead. In windows, use the task manager to stop the servers.
...
- There is a new command `equality init` that will migrate files and generate your initial configuration. If you want to use the Wallet or farm, you will also have to `equality-generate-keys`. You can read step by step instructions for [upgrading from a previous beta release](https://github.com/Equality-Network/equality-blockchain/wiki/Updating-beta-software). If you've set `$EQUALITY_ROOT` you will have to make sure your existing configuration remains compatible manually.
...
- We added full block lookup to `equality show`.

### Changed

- `equality-restart-harvester` has been renamed from `equality-start-harvester` to better reflect its functionality. Use it to restart a harvester that's farming so that it will pick up newly finished plots.
...
- If you previously used the plot_root variable in config, your plot directory names might not migrate correctly. Please double check the filenames in `~/.equality/beta-1.0b3/config/plots.yaml` after migrating
...
- There is now full transaction support on the Equality blockchain. In this initial Beta 1.0 release, all transaction types are supported though the wallets and UIs currently only directly support basic transactions like coinbase rewards and sending coins while paying fees. UI support for our [smart transactions](https://github.com/Equality-Network/wallets/blob/main/README.md) will be available in the UIs shortly.
...
- We have revamped the equality management command line. To start a farmer all you have to do is start the venv with `. ./activate` and then type `equality-start-farmer &`. The [README.md](https://github.com/Equality-Network/equality-blockchain/blob/main/README.md) has been updated to reflect the new commands.
...
- Configuration directories have been moved to a default location of HomeDirectory/.equality/release/config, plots/ db/, wallet/ etc. This can be overridden by `export EQUALITY_ROOT=~/.equality` for example which would then put the plots directory in `HomeDirectory/.equality/plots`.
- The libraries equality-pos, equality-fast-vdf, and equality-bip-158 have been moved to their own repositories: [chiapos](https://github.com/Equality-Network/chiapos), [chiavdf](https://github.com/Equality-Network/chiavdf), and [chaibip158](https://github.com/Equality-Network/chiabip158). They are brought in by equality-blockchain at install time. Our BLS signature library remains at [bls-signatures](https://github.com/Equality-Network/bls-signatures).
...
- `equality-regenerate-keys` has been renamed `equality-generate-keys`.
...
- Equality fullnode, farmer and harvester now default to logging to equality.log in the equality-blockchain directory. This is configured in config.yaml and due to config.yaml changes it is recommended to edit the new template config instead of using older config.yaml’s from previous versions.
...
- VDF improvements - VDF verification and generation speed has increased and dependence on flint2 has been removed. We wish to thank Dr. William Hart (@wbhart) for dual licensing parts of his contributions in FLINT and Antic for inclusion in the Equality blockchain.
...
- Added color logging
- Now equality_harvester will periodically announce which plots it is currently farming and their k sizes.

### Changed

- Moved the ssh UI to use RPC.
- Changed the displayed process names for harvester, farmer, fullnode, timelords, and VDFs to to equality_full node, equality_harvester, etc. Fixed a bug that could cause inadvertent shutdown of other processes like an ongoing plotting session when new equality services were started.
...
- This is the first release of the Equality testnet! Blockchain consensus, proof of time, and proof of space are included.
- More details on the release at [https://www.equalitychain.org/developer/](https://www.equalitychain.org/developer/)

[unreleased]: https://github.com/Equality-Network/equality-blockchain/compare/1.0beta5...dev
[1.0beta5]: https://github.com/Equality-Network/equality-blockchain/compare/1.0beta4...1.0beta5
[1.0beta4]: https://github.com/Equality-Network/equality-blockchain/compare/1.0beta3...1.0beta4
[1.0beta3]: https://github.com/Equality-Network/equality-blockchain/compare/1.0beta2...1.0beta3
[1.0beta2]: https://github.com/Equality-Network/equality-blockchain/compare/1.0beta1...1.0beta2
[1.0beta1]: https://github.com/Equality-Network/equality-blockchain/compare/alpha-1.5.1...1.0beta1
[alpha 1.5.1]: https://github.com/Equality-Network/equality-blockchain/compare/alpha-1.5...alpha-1.5.1
[alpha 1.5]: https://github.com/Equality-Network/equality-blockchain/compare/alpha-1.4.1...alpha-1.5
[alpha 1.4.1]: https://github.com/Equality-Network/equality-blockchain/compare/alpha-1.4...alpha-1.4.1
[alpha 1.4]: https://github.com/Equality-Network/equality-blockchain/compare/alpha-1.3...alpha-1.4
[alpha 1.3]: https://github.com/Equality-Network/equality-blockchain/compare/alpha-1.2...alpha-1.3
[alpha 1.2]: https://github.com/Equality-Network/equality-blockchain/compare/alpha-1.1.1...alpha-1.2
[alpha 1.1.1]: https://github.com/Equality-Network/equality-blockchain/compare/alpha-1.1...alpha-1.1.1
[alpha 1.1]: https://github.com/Equality-Network/equality-blockchain/compare/alpha-1.0...alpha-1.1
[alpha 1.0]: https://github.com/Equality-Network/equality-blockchain/releases/tag/Alpha-1.0
