...
from equality.daemon.server import WebSocketServer, create_server_for_daemon, daemon_launch_lock_path, singleton
from equality.full_node.full_node_api import FullNodeAPI
from equality.server.start_farmer import service_kwargs_for_farmer
from equality.server.start_full_node import service_kwargs_for_full_node
from equality.server.start_harvester import service_kwargs_for_harvester
from equality.server.start_introducer import service_kwargs_for_introducer
from equality.server.start_service import Service
from equality.server.start_timelord import service_kwargs_for_timelord
from equality.server.start_wallet import service_kwargs_for_wallet
from equality.simulator.start_simulator import service_kwargs_for_full_node_simulator
from equality.timelord.timelord_launcher import kill_processes, spawn_process
from equality.types.peer_info import PeerInfo
from equality.util.bech32m import encode_puzzle_hash
from equality.util.block_tools import BlockTools, test_constants
from equality.util.hash import std_hash
from equality.util.ints import uint16, uint32
...
async def setup_farmer(
    ...
    config["xeq_target_address"] = encode_puzzle_hash(b_tools.farmer_ph, "xeq")
    config["pool_public_keys"] = [bytes(pk).hex() for pk in b_tools.pool_pubkeys]
    config["port"] = port
    config_pool["xeq_target_address"] = encode_puzzle_hash(b_tools.pool_ph, "xeq")
