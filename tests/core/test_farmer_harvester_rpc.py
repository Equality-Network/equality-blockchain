...
from equality.plotting.plot_tools import stream_plot_info_ph, stream_plot_info_pk
from equality.protocols import farmer_protocol
from equality.rpc.farmer_rpc_api import FarmerRpcApi
from equality.rpc.farmer_rpc_client import FarmerRpcClient
from equality.rpc.harvester_rpc_api import HarvesterRpcApi
from equality.rpc.harvester_rpc_client import HarvesterRpcClient
from equality.rpc.rpc_server import start_rpc_server
...
from equality.util.block_tools import get_plot_dir
...
from equality.wallet.derive_keys import master_sk_to_wallet_sk
...
class TestRpc:
    ...
    async def test1(self, simulation):
        ...
        try:
            ...
            await client.set_reward_targets(encode_puzzle_hash(new_ph, "xeq"), encode_puzzle_hash(new_ph_2, "xeq"))
            ...
            await client.set_reward_targets(None, encode_puzzle_hash(new_ph_3, "xeq"))
            ...
            assert config["farmer"]["xeq_target_address"] == encode_puzzle_hash(new_ph, "xeq")
            assert config["pool"]["xeq_target_address"] == encode_puzzle_hash(new_ph_3, "xeq")

            new_ph_3_encoded = encode_puzzle_hash(new_ph_3, "xeq")
