...
from equality.protocols import full_node_protocol, introducer_protocol
from equality.protocols.protocol_message_types import ProtocolMessageTypes
from equality.server.address_manager import AddressManager, ExtendedPeerInfo
from equality.server.address_manager_store import AddressManagerStore
...
from equality.util.ints import uint64
...
class FullNodeDiscovery:
    def __init__(
        ...
        self.server: EqualityServer = server
    ...
    async def on_connect(self, peer: ws.WSEqualityConnection):
        if (
    ...
    async def update_peer_timestamp_on_message(self, peer: ws.WSEqualityConnection):
        if (
            peer.is_outbound
            and peer.peer_server_port is not None
            and peer.connection_type is NodeType.FULL_NODE
    ...
    async def _introducer_client(self):
        ...
        async def on_connect(peer: ws.WSEqualityConnection):
