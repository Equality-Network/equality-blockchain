...
from equality.cmds.init_funcs import equality_full_version_str
from equality.protocols.protocol_message_types import ProtocolMessageTypes
from equality.protocols.shared_protocol import Capability, Handshake
from equality.server.outbound_message import Message, NodeType, make_msg
from equality.server.rate_limits import RateLimiter
...
from equality.util.ints import uint8, uint16

# Each message is prepended with LENGTH_BYTES bytes specifying the length
from equality.util.network import class_for_type, is_localhost

# Max size 2^(8*4) which is around 4GiB
LENGTH_BYTES: int = 4


class WSEqualityConnection:
    """
    ...
    async def perform_handshake(self, network_id: str, protocol_version: str, server_port: int, local_type: NodeType):
        if self.is_outbound:
            ...
            if ProtocolMessageTypes(inbound_handshake_msg.type) != ProtocolMessageTypes.handshake:
                raise ProtocolError(Err.INVALID_HANDSHAKE)
            if inbound_handshake.network_id != 'equality-' + network_id:
