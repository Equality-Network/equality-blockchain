...
from equality.protocols import wallet_protocol
from equality.protocols.full_node_protocol import RequestProofOfWeight, RespondProofOfWeight
from equality.protocols.protocol_message_types import ProtocolMessageTypes
from equality.protocols.wallet_protocol import (
...
from equality.server.node_discovery import WalletPeers
...
from equality.util.byte_types import hexstr_to_bytes
from equality.util.errors import Err, ValidationError
from equality.util.ints import uint32, uint128
from equality.util.keychain import Keychain
from equality.util.lru_cache import LRUCache
from equality.util.merkle_set import MerkleSet, confirm_included_already_hashed, confirm_not_included_already_hashed
from equality.util.path import mkdir, path_from_root
from equality.wallet.block_record import HeaderBlockRecord
from equality.wallet.derivation_record import DerivationRecord
from equality.wallet.settings.settings_objects import BackupInitialized
from equality.wallet.transaction_record import TransactionRecord
from equality.wallet.util.backup_utils import open_backup_file
from equality.wallet.util.wallet_types import WalletType
from equality.wallet.wallet_action import WalletAction
from equality.wallet.wallet_blockchain import ReceiveBlockResult
from equality.wallet.wallet_state_manager import WalletStateManager
...
class WalletNode:
    ...
    server: Optional[EqualityServer]
    log: logging.Logger
    ...
    def get_key_for_fingerprint(self, fingerprint: Optional[int]):
        ...
        if len(private_keys) == 0:
            self.log.warning("No keys present. Create keys with the UI, or with the 'equality keys' program.")
            return None
    ...
    async def on_connect(self, peer: WSEqualityConnection):
        if self.wallet_state_manager is None or self.backup_initialized is False:
            return None
    ...
    async def complete_blocks(self, header_blocks: List[HeaderBlock], peer: WSEqualityConnection):
    ...
    async def new_peak_wallet(self, peak: wallet_protocol.NewPeakWallet, peer: WSEqualityConnection):
        if self.wallet_state_manager is None:
            return None
    ...
    async def _sync(self) -> None:
        ...
        peers: List[WSEqualityConnection] = self.server.get_full_node_connections()
        if len(peers) == 0:
    ...
    async def get_additions(self, peer: WSEqualityConnection, block_i, additions) -> Optional[List[Coin]]:
        if len(additions) > 0:
    ...
    async def get_removals(self, peer: WSEqualityConnection, block_i, additions, removals) -> Optional[List[Coin]]:
        assert self.wallet_state_manager is not None
