...
from equality.consensus.cost_calculator import calculate_cost_of_program, NPCResult
from equality.full_node.bundle_tools import simple_solution_generator
...
from equality.types.announcement import Announcement
...
from equality.wallet.puzzles.p2_delegated_puzzle_or_hidden_puzzle import (
    DEFAULT_HIDDEN_PUZZLE_HASH,
    calculate_synthetic_secret_key,
    puzzle_for_pk,
    solution_for_conditions,
)
from equality.wallet.puzzles.puzzle_utils import (
...
from equality.wallet.secret_key_store import SecretKeyStore
from equality.wallet.sign_coin_solutions import sign_coin_solutions
...
from equality.wallet.wallet_info import WalletInfo
...
class Wallet:
    ...
    async def create_spend_bundle_relative_equality(self, equality_amount: int, exclude: List[Coin]) -> SpendBundle:
        list_of_solutions = []
        utxos = None

        # If we're losing value then get coins with at least that much value
        # If we're gaining value then our amount doesn't matter
        if equality_amount < 0:
            utxos = await self.select_coins(abs(equality_amount), exclude)
        else:
        ...
        equality_amount = spend_value + equality_amount
        ...
        for coin in utxos:
            ...
            if output_created is None:
                ...
                primaries = [{"puzzlehash": newpuzhash, "amount": equality_amount}]
