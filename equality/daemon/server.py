...
from equality.cmds.init_funcs import equality_init
from equality.daemon.windows_signal import kill
from equality.server.server import ssl_context_for_root, ssl_context_for_server
from equality.ssl.create_ssl import get_mozzila_ca_crt
from equality.util.equality_logging import initialize_logging
from equality.util.config import load_config
from equality.util.json_util import dict_to_json_str
from equality.util.path import mkdir
from equality.util.service_groups import validate_service
from equality.util.setproctitle import setproctitle
from equality.util.ws_message import WsRpcMessage, create_payload, format_response
...
try:
    ...
    print("Error: Make sure to run . ./activate from the project folder before starting Equality.")
    quit()
...
service_plotter = "equality plots create"
...
if getattr(sys, "frozen", False):
    name_map = {
        "equality": "equality",
        "equality_wallet": "start_wallet",
        "equality_full_node": "start_full_node",
        "equality_harvester": "start_harvester",
        "equality_farmer": "start_farmer",
        "equality_introducer": "start_introducer",
        "equality_timelord": "start_timelord",
        "equality_timelord_launcher": "timelord_launcher",
        "equality_full_node_simulator": "start_simulator",
    }
...
class WebSocketServer:
    ...
    async def exit(self) -> Dict[str, Any]:
        ...
        log.info("equality daemon exiting in 5 seconds")
...
def launch_plotter(root_path: Path, service_name: str, service_array: List[str], id: str):
    # we need to pass on the possibly altered EQUALITY_ROOT
    os.environ["EQUALITY_ROOT"] = str(root_path)
...
def launch_service(root_path: Path, service_command) -> Tuple[subprocess.Popen, Path]:
    ...
    log.debug(f"Launching service with EQUALITY_ROOT: {os.environ['EQUALITY_ROOT']}")
...
async def async_run_daemon(root_path: Path) -> int:
    equality_init(root_path)
    config = load_config(root_path, "config.yaml")
    setproctitle("equality_daemon")
